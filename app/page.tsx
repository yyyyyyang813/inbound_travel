...
function CheckoutView({ tour }: { tour: Tour }) {
  const [sent, setSent] = useState(false); const [payment, setPayment] = useState("card"); const booking = typeof window !== "undefined" ? JSON.parse(window.localStorage.getItem("arctic-tern-booking") || "{}") : {}; const guests = booking.guests || 2;
  ...
  return <div className="checkout-page"><button className="back" onClick={() => go("tour", tour.id)}>← Back to experience</button><div className="checkout-heading"><p className="eyebrow">Secure request · Arctic Tern</p><h1>One last step.</h1><p>Share how we can reach you. We’ll confirm with your Buddy before any payment.</p></div><form className="checkout-grid" onSubmit={(e: FormEvent) => { e.preventDefault(); setSent(true); window.scrollTo({ top: 0 }); }}><div className="contact-form"><h2>Contact details</h2><p>Used only to confirm this booking.</p><div className="form-two"><label>Full name<input required placeholder="Your name"/></label><label>WhatsApp number<input required placeholder="+1 202 555 0182"/></label></div><label>Email address<input required type="email" placeholder="you@example.com"/></label><label>Special requests<textarea placeholder="Dietary needs, children, accessibility…"/></label><h2 className="payment-title">Preferred payment</h2><p>You’ll receive a secure link only after confirmation.</p><div className="payment-options"><button type="button" className={payment === "card" ? "chosen" : ""} onClick={() => setPayment("card")}>◉ &nbsp; Credit or debit card</button><button type="button" className={payment === "paypal" ? "chosen" : ""} onClick={() => setPayment("paypal")}>P &nbsp; PayPal</button></div></div><aside className="order-card"><img src={tour.image} alt=""/><div className="mini-host"><img src={tour.hostImage} alt=""/><span>with {tour.host}</span></div><p className="eyebrow">Your experience</p><h3>{tour.title}</h3><div className="order-lines"><span>Date <b>{booking.date || "Flexible"}</b></span><span>Guests <b>{guests}</b></span><span>Experience <b>${tour.price * guests}</b></span><span>Booking fee <b>$0</b></span></div><div className="order-total"><span>Estimated total</span><strong>${tour.price * guests} USD</strong></div><button className="primary" type="submit">Send booking request</button><small>By continuing, you agree to our booking terms and privacy policy.</small></aside></form></div>;
}
